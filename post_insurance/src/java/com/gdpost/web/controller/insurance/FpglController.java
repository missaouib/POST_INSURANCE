...
public class FpglController {
	...
	private static final String CREATE = "insurance/fpgl/wtj/create";
	private static final String UPDATE = "insurance/fpgl/wtj/update";
	private static final String LIST = "insurance/fpgl/wtj/list";
	...
	public Policy preload(@RequestParam(value = "id", required = false) Long id) {
		if (id != null) {
			Policy user = fpglService.get(id);
			if(user != null) {
	...
	public @ResponseBody String delete(@PathVariable Long id) {
		Policy user = null;
		try {
			user = fpglService.get(id);
			fpglService.delete(user.getId());
	...
	public @ResponseBody String deleteMany(Long[] ids) {
		...
		try {
			for (int i = 0; i < ids.length; i++) {
				Policy user = fpglService.get(ids[i]);
				fpglService.delete(user.getId());
	...
	public String list(ServletRequest request, Page page, Map<String, Object> map) {
		...
		List<Policy> users = fpglService.findByExample(specification, page);
