...
public interface UnderWriteDAO extends JpaRepository<UnderWrite, Long>, JpaSpecificationExecutor<UnderWrite> {
	...
	List<UnderWrite> findDistinctUnderWrite2Pop(@Param("orgCode") String orgCode, @Param("status") String status, Pageable pageable);
	...
	@Query("select t1 from UnderWrite t1,Organization t2 where t1.organization.id=t2.id and "
					...
					+ "and t2.orgCode like :orgCode and "
					+ "t1.signInputDate is null and t1.status=:status")
	List<UnderWrite> findDistinctUnderWrite2Call(@Param("orgCode") String orgCode, @Param("status") String status, Pageable pageable);
	...
	List<UnderWrite> findDistinctUnderWrite2Weixin(@Param("orgCode") String orgCode, @Param("status") String status, Pageable pageable);
}