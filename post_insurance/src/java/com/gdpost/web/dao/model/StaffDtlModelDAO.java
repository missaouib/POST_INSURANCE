...
public interface StaffDtlModelDAO extends JpaRepository<StaffDtlModel, String>, JpaSpecificationExecutor<StaffDtlModel> {
	...
	@Query(name="getProvAllStaffDetailWithBankCode",
			...
			+ "where tp.cs_flag<>1 and tp.attached_flag=0 and tp.policy_no=tpd.policy_no and tpd.holder_card_num=ts.id_card and tp.bank_code=tbc.cpi_code and year(tp.policy_date)=year(now()) and year(tp.policy_date)=ts.year and tp.fee_frequency like :toPerm "
			...
			nativeQuery=true)
	List<StaffDtlModel> getProvAllStaffDetailWithBankCode(@Param("orgCode")String orgCode, @Param("p1")String pd1, @Param("p2")String pd2, @Param("netFlag")String netFlag, @Param("prdCode")String prdCode, @Param("toPerm")String toPerm, @Param("duration")Integer duration);
