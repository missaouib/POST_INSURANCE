...
public interface DocStatModelDAO extends JpaRepository<DocStatModel, String>, JpaSpecificationExecutor<DocStatModel> {
	@Query(nativeQuery=true,
			...
			+ "where dns.organ_name=tob.short_name and dns.sign_date between :d1 and :d2 group by left(dns.organ_name,2) order by tob.org_code; ")
	List<DocStatModel> getDocNotScanStat(@Param("d1")String d1, @Param("d2")String d2);
	
	@Query(nativeQuery=true,
			name="getSubDocNotScanStat",
			value="select tob.short_name as org_name,count(dns.policy_no) as sum_doc "
			+ "from t_doc_not_scan_dtl dns, t_organization tob "
			+ "where dns.organ_name=tob.short_name and LEFT(organ_name,2)=:organName and dns.sign_date between :d1 and :d2 "
			+ "group by tob.short_name order by tob.org_code;")
	List<DocStatModel> getSubDocNotScanStat(@Param("organName")String organName, @Param("d1")String d1, @Param("d2")String d2);
	
	@Query(nativeQuery=true,
			name="getDocSumStat",
			value="select left(tp.organ_name,2) as org_name,count(tp.policy_no) as sum_doc "
			+ "from t_policy tp, t_organization tob "
			+ "where tp.policy_no like \"8644%\" and tp.organ_name=tob.short_name and LEFT(tp.organ_name,2)=:organName "
