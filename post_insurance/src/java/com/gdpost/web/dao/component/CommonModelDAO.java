...
public interface CommonModelDAO extends JpaRepository<CommonModel, Long>, JpaSpecificationExecutor<CommonModel> {
	...
	@Query(name="getProvTuiBaoWarningWithPolicyDateAndCsDate",
			value="select left(tp.organ_name,2) as organ_name,sum(policy_fee) as policy_fee,"
			+ "(select sum(t4.policy_fee) from t_policy t4, t_bank_code t5  where t4.bank_code=t5.cpi_code and t4.organ_code=tp.organ_code and t4.policy_date between :p1 and :p2 and t5.flag=:flag and t4.organ_code like :orgCode and t4.prod_code like :prdCode ) as sum_policy_fee "
	...
	@Query(name="getTuiBaoWarningWithPolicyDateAndCsDate",
			...
			+ "order by tp.organ_code;",
			nativeQuery=true)
	List<CommonModel> getTuiBaoWarningWithPolicyDateAndCsDate(@Param("orgCode")String orgCode, @Param("p1")String pd1, @Param("p2")String pd2, @Param("c1")String csd1, @Param("c2")String csd2, @Param("flag")String flag, @Param("prdCode")String prdCode);
}