...
<div id="divZZL" style="padding-left:100px;padding-top:20px;">
	...
	店名
	<select id="listDM" name="dm" style="width:100px;" onchange="onChange(this.value);">
		<c:forEach var="item" items="${dm}">
		<option value="${item}">${item}</option>
		</c:forEach>
	</select>
	品类
	<select id="listPL" name="pl" style="width:100px;" onchange="onChange(this.value);">
	...
	<input type="radio" name="statisticstype" value="3" onclick="onChange(this.value);" />增长率
	<input type="radio" name="statisticsorder" value="0" onclick="onChange(this.value);" checked="checked" />顺序
	<input type="radio" name="statisticsorder" value="1" onclick="onChange(this.value);"/>倒序
	<a style="display:none;" class="btn btn-success" id="btnOK" onclick="javascript:onStatistics();"><i class="icon-ok icon-white"></i>统计</a>
	<div id="divChart"></div>
</div>
...
var data = [{name: 'pm1',data: [7.0, 6.9, 9.5]}, {name: 'pm2',data: [3.9, 4.2, 5.7]}];
...
function onStatistics() {
	...
	var statisticsorder = $("input[name='statisticsorder']:checked").val();
	$.ajax({
		type: 'post',
		url: "/uploaddatamanage/statistics/pp",
		dataType: "text",
		data: {"ny": ny, "dm": dm, "pl": pl, "statisticstype": statisticstype, "statisticsorder": statisticsorder},
		error: function () {
