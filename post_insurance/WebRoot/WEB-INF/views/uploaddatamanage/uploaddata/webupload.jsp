...
<div id="divUploadData" class="pageContent" layoutH="0">
	<div style="padding-left:60px;padding-top:30px; height:400px;">
		<div>
			<img src="${contextPath}/download/shuoming.png" />
		</div>
		<div id="uploadContainer" style="width:800px;">
		    ...
		    <a href="${contextPath}/download/template.xls" target="_blank">上传模板xls格式下载</a>&nbsp;&nbsp;&nbsp;&nbsp;
		    <a href="${contextPath}/download/template.xlsx" target="_blank">上传模板xlsx格式下载</a>
		</div>
		<br />
		<span id="lblBrowser" style="display:none;"><a target=_blank href='http://www.microsoft.com/zh-cn/download/internet-explorer-8-details.aspx'>您的浏览器版本太低，请点击下载新版本浏览器。</a></span>
		<span id="lblStarter" style="display:;"><a target=_blank href='${contextPath}/download/411.bat'>为保证正常使用系统的所有功能，请下载专用浏览器启动文件访问系统。</a></span>
		<div>
			<p>
				<label>备注：</label>
				<textarea id="txtMemo" rows="3" cols="100" maxlength="200"></textarea>
			</p>
		</div>

		<pre id="console"></pre>
		<br />
		<div id="filelist" style="background:${contextPath}/download/shuoming.png">Your browser doesn't have Flash, Silverlight or HTML5 support.</div>
	</div>
</div>
        ...
        init: {
            PostInit: function () {
                ...
                document.getElementById('uploadfiles').onclick = function () {
                	...
                	if(ny == ${currentNY} && ${today}<20) {
                		if(!window.confirm("现在应该上传的是" + ${lastNY} + "的数据，你当前选择的数据月份是" + ny + "，确定上传吗？")) {
                			return false;
                		}
                	}
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if(response.result == "confirm") {
                                    ...
                                    alert("数据文件导入失败，请联系管理员，电话020-37886729,37886776");
                                }
                            }
                        });
                    } else {
            ...
            Error: function (up, err) {
				...
				alert("数据文件上传失败，请联系管理员，电话020-37886729,37886776");
...
if(navigator.userAgent.indexOf("MSIE")>0){   
    if(navigator.userAgent.indexOf("MSIE 6.0")>0){
      // ie6
      document.getElementById("lblBrowser").style.display = '';
	...
	//document.getElementById("lblStarter").style.display = '';
	alert("为保证正常使用系统的所有功能，请下载专用浏览器启动文件访问系统。");
}
</script>
