...
<dwz:paginationForm action="${contextPath }/qygl/issue/reuseRisk/list" page="${page }">
...
<form id="qyWriteForm" method="post" action="${contextPath }/qygl/issue/reuseRisk/list" onsubmit="return navTabSearch(this)">
		...
		<table class="searchContent">
				...
				<tr>
					...
					<form:select path="issue.keyInfo" id="cwki" class="combox">
							...
							<form:option value="地址">地址</form:option>
							<form:option value="手机号码">电话</form:option>
						</form:select>
					</td>
					<td>&nbsp;</td>
				</tr>
	...
	<div class="panelBar">
		<ul class="toolBar">
			<shiro:hasPermission name="CheckWrite:view">
				<li><a iconClass="magnifier" target="dialog" rel="lookup2organization_edit" mask="true" width="820" height="520" href="${contextPath }/qygl/issue/reuseRisk/view/{slt_uid}"><span>处理详情</span></a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="CheckWrite:edit">
				<li class="line">line</li>
				<li><a class="edit" target="dialog" rel="lookup2organization_edit" mask="true" width="820" height="520" href="${contextPath }/qygl/issue/reuseRisk/update/{slt_uid}"><span>回复</span></a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="CheckWrite:provEdit">
				<li><a class="edit" target="ajaxTodo" href="${contextPath }/qygl/issue/reuseRisk/appeal/{slt_uid}" title="确认并核实可申诉?"><span>申诉</span></a></li>
				<li><a class="edit" target="ajaxTodo" href="${contextPath }/qygl/issue/reuseRisk/delete/{slt_uid}" title="确认要物理删除记录?"><span>删除</span></a></li>
				<li><a class="edit" target="selectedTodo" rel="ids" href="${contextPath }/qygl/issue/reuseRisk/close" title="确认已完成整改?"><span>确认整改完毕</span></a></li>
				<li><a class="edit" target="ajaxTodo" href="${contextPath }/qygl/issue/reuseRisk/fail/{slt_uid}" title="确认无法整改?"><span>无法整改</span></a></li>
			</shiro:hasPermission>
			<li><a class="icon" target="_blank" href="${contextPath }/qygl/issue/reuseRisk/toXls?checker=${checker }&keyInfo=${keyInfo }&fixStatus=${status }&orgCode=${orgCode }&search_GTE_policy.policyDate=${search_GTE_policy_policyDate}&search_LTE_policy.policyDate=${search_LTE_policy_policyDate}&search_GTE_replyTime=${param.search_GTE_replyTime}&search_LTE_replyTime=${param.search_LTE_replyTime}&search_GTE_checkBatch=${param.search_GTE_checkBatch}&search_LTE_checkBatch=${param.search_LTE_checkBatch}"><span>导出Excel</span></a></li>
			<li class="line">line</li>
