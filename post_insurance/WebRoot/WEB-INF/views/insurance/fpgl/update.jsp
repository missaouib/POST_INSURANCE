...
<form method="post" action="${contextPath}/fpgl/update" class="required-validate pageForm" onsubmit="return validateCallback(this, dialogReloadNavTab);">
	...
	<div class="formBar">
		<ul>
			<li><div class="button" <c:if test='${issue.status eq "已关闭" }'>class="buttonDisabled"</c:if> <c:if test='${issue.status ne "已关闭" }'>class="button"</c:if>><div class="buttonContent"><button type="submit" <c:if test='${issue.status eq "已关闭" }'>disabled=true</c:if>>确定</button></div></div></li>
