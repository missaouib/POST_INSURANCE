...
function mJumpClick($this, isChecked) {
	var $thisAllchk = $this.next("span[ref='mAllChk']");
	if (isChecked) {
		$this.attr("class", "button mchk checkbox_true_full");
		$thisAllchk.attr("class", "button mchk checkbox_false_full");
		$thisAllchk.click();
		$thisAllchk.attr("class", "button mchk checkbox_true_full");
	} else {
		$this.attr("class", "button mchk checkbox_false_full");
		$thisAllchk.attr("class", "button mchk checkbox_true_full");
		$thisAllchk.click();
		$thisAllchk.attr("class", "button mchk checkbox_false_full");
	}
	...
	var $cTr = $("tr[pid='" + id + "']");
	var $cTreeChk = $("span[ref='mTreeChk']", $cTr);
	var $cAllChk = $("span[ref='mAllChk']", $cTr);
	...
	$cTr.each(function(){
		...
		if ($ccTr.length > 0) {
			$ccTr.each(function(){
				var $c = $("span[ref='mTreeChk']", $ccTr);
				mJumpClick($c, isChecked);			
			});
		}
	});
}

function initTblMemberRolePage() {
	// 给全选加入事件
    $(".mSetAll").click(function(){
    	var isChecked = null;
    	...
    	if (chkClass.indexOf("true") > -1) {
    		...
    		$(this).attr("class", "button mchk checkbox_false_full mSetAll");
    	} else {
    		isChecked = true;
    		
    		$(this).attr("class", "button mchk checkbox_true_full mSetAll");
    	}
    	 
    	var $td = $(this).parent().nextAll("td");
    	var $inputSpan = $(".inputValueTblMemberRole", $td);
		$("input[type=checkbox]", $inputSpan).each(function(){
			if (isChecked == true) {
				$(this).attr("checked", "checked");
			} else {
	...
	// 初始化permission
	$(".inputValueTblMemberRole input[type=checkbox]").click(function(){
		var isChecked = $(this).is(":checked");
		var $td = $(this).parent().parent().prevAll("td");
		var $mchk = $(".mSetAll", $td);
		if (isChecked == false) {
			$mchk.attr("class", "button mchk checkbox_false_full mSetAll");
		}
	});
	//初始化treeTable
	var option = {
    ...
    $("span[class*='checkbox_']").each(function(){  	
    	$(this).mouseover(function(){
    		var chkClass = $(this).attr("class").split(" ")[2];
    		if(chkClass.indexOf("_focus") < 0) {
    			if ($(this).attr("class").indexOf("mSetAll") > -1) {
    				$(this).attr("class", "button mchk " + chkClass + "_focus" + " mSetAll");    				
    			} else {
    				$(this).attr("class", "button mchk " + chkClass + "_focus");
    			}
        	}
		});
    	...
    	$(this).mouseout(function(){
    		...
    		if(chkClass.indexOf("_focus") > -1) {
    			if ($(this).attr("class").indexOf("mSetAll") > -1) {
    				$(this).attr("class", "button mchk " + chkClass.substring(0, chkClass.length-6) + " mSetAll");    				
    			} else {
    				$(this).attr("class", "button mchk " + chkClass.substring(0, chkClass.length-6));
    			}
        	}
		});

    	// 给treeTable的chk添加click事件
    	if ($(this).attr("class").indexOf("mSetAll") < 0) {
        	$(this).click(function(){
            	var isChecked = null;
            	var chkClass = $(this).attr("class").split(" ")[2];
            	if (chkClass.indexOf("true") > -1) {
